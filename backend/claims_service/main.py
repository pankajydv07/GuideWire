...
from claims_service.consumer import start_consumer

logging.basicConfig(
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    level=getattr(logging, settings.LOG_LEVEL),
)
logger = logging.getLogger("zylo.claims_service")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "claims_service"}
